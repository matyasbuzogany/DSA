...
void testIterator() {
	...
	cout << "Elemts vector size : " << elements.size() << endl;

	assert(elements.size() == b4.size());
	for (unsigned int i = 0; i < elements.size(); i++) {
		TElem lastElem = elements.at(elements.size() - i - 1);
		cout << "Current element: " << lastElem << " -> search function returned: " << b4.search(lastElem) << endl;
	...
	for (unsigned int i = 0; i < elements2.size(); i++) {
		...
		cout << "Current firstElem: " << firstElem << " -> search function returned: " << b5.search(firstElem) << endl;
