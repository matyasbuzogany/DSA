...
void testIteratorSteps(SortedMap& m, Relation r) {
    SortedMapIterator li = m.iterator();
...
//        cout << "Elem1 : " << elem.first << ", Elem2: " << elem2.first << endl;
        assert(r(elem2.first, elem.first));
        elem = elem2;
        count++;
        li.next();
    }
...
//    sm.toString();
    int intervalDim = 10;
    for (int i = cMin; i <= cMax; i++) {
//        cout << "i = " << i << " , Search: " << sm.search(i) << " , i+1: " << i + 1 << endl;
...
void populateSMEmpty(SortedMap& sm, int cMin, int cMax) {
    ...
    int n = keys.size();
    for (int i = 0; i < n; i++) {
//        cout << sm.add(keys[i], keys[i]) << endl;
...
void testIterator(Relation r) {
    ...
    SortedMapIterator itSM = sm.iterator();
    assert(itSM.valid());
    itSM.first();
    assert(itSM.valid());
